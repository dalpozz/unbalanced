...
function(X, Y, type = "ubSMOTE", positive = 1, 
    ...
    Y <- factor(Y == positive, labels = c(0, 1))
    ...
    if (type == "ubOver") 			data <- ubOver(X, Y, k)
    ...
    if (verbose) {
        cat("Proportion of positives after '", type, "' :", round(length(which(Y == 
            1))/N * 100, digits = 2), "% of", N, "total instances \n")
    }
    
    #transform the outout with the original labels
    Y <- factor(Y == 1, labels = lev)
